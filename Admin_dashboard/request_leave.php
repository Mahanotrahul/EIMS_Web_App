...
else
{
	...
	if(!empty($row['EMAIL']))
	{
		 ...
		 if(($_SERVER["REQUEST_METHOD"]== "POST") && (isset($_POST['submit_leave_request'])))
		 {
			...
			else if(strcmp($password, $orig_password) == 0)
			{
				...
				else
				{
					...
					if($query)
					{
						echo  "<script type='text/javascript'>alert('Succesfully Applied for Leave. Leaves are not granted unless approved by an Incharge.');</script>";
					}
					else
					{
		 ...
		 else if(($_SERVER["REQUEST_METHOD"]== "POST") && (isset($_POST['remove_leave_request_submit'])))
		 {
												...
												<div class="panel-body">
													...
													while($row_leave_requests_query = mysqli_fetch_assoc($leave_requests_query))
													{
														...
														echo	'<td><form role="form" action="'.htmlspecialchars($_SERVER["PHP_SELF"]).'" method="post">
																			...
																			<input type="hidden" value="'.$row_leave_requests_query['Time'].'" name="applied_time">
																			<input type="hidden" value="'.$row_leave_requests_query['Date'].'" name="applied_date">
