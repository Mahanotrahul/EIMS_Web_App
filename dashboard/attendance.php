	...
	if(!empty($row['EMAIL']))
	{
		 ...
		 if(($_SERVER["REQUEST_METHOD"]== "POST") && (isset($_POST['CheckIn'])))
		 {
			 ...
			 if(mysqli_num_rows($present_incharge_query) == 0)
			 {
				 ...
				 $incharge_mem_id = -1; // -1 means Incharge was not present for this location at the the time of giving attendance
			 }
			 else
			 {
								...
								<div class="row">
									<div id ="headh" class="col-md-6">
										<div class="form-group">
											...
											?>
											<div style="color:#ffffff; font-family:Calibri;">
											<?php
												$loc_query = mysqli_query($con, "SELECT Loc FROM locations WHERE LOC_ID = '".$_SESSION['mem_loc_id']."'");
												$row_loc_query = mysqli_fetch_assoc($loc_query);
												$present_incharge_query = mysqli_query($con, "SELECT * FROM present_incharge WHERE LOC_ID = '".$_SESSION['mem_loc_id']."'");
												if(mysqli_num_rows($present_incharge_query) == 0)
												{
													// If no incharge exists for this member's location;
													echo '<br>Your Attendance will be approved by the incharge. Presently there is no incharge for your location.';
												}
												else
												{
													// If incharge exists for this member's location;
													$row_present_incharge_query = mysqli_fetch_assoc($present_incharge_query);
													$incharge_mem_id = $row_present_incharge_query['MEM_ID'];
													if(!($mem_id == $incharge_mem_id))
													{
														$incharge_name_query = mysqli_query($con, "SELECT FNAME, LNAME FROM member WHERE ID = '$incharge_mem_id'");
														$row_incharge_name_query = mysqli_fetch_assoc($incharge_name_query);
														$incharge_name = $row_incharge_name_query['FNAME']." ".$row_incharge_name_query['LNAME'];
														echo '<br>Your Attendance will be approved by the incharge. <br>Present Incharge for your location - '.$row_loc_query['Loc'].' is '.$_SESSION['salutation']." ".$incharge_name;
													}
												}
